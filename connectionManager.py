...
import socket, socketserver, _thread, threading
from PyQt5.QtCore import pyqtSignal, QObject

class Signal(QObject):
    dataRecieved = pyqtSignal()
    pass
        ...
        while True:
            try:
                ...
                if message.strip().find('#id') == 0:
                    status = ('id', None)
                    self.signal.dataRecieved.emit()
                    pass
                elif message.strip().find('#add') == 0:
                    status = ('add', message.strip()[5:], None)
                    self.signal.dataRecieved.emit()
                    pass
                elif message.strip().find('#del') == 0:
                    status = ('del', message.strip()[5:], None)
                    self.signal.dataRecieved.emit()
                    pass
                elif message.strip().find('#change') == 0:
                    status = ('change', (message.strip()[8:].split(' ')[0], message.strip()[8:].split(' ')[1])))
                    self.signal.dataRecieved.emit()
                    pass
                elif message.strip().find('#list') == 0:
                    status = ('list', messsage.strip()[6:].split(' ')))
                    self.signal.dataRecieved.emit()
                    pass
                else:
